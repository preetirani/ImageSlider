...
open class ImageSlideshow: UIView {
    ...
    private func layoutProgressView() {
        ...
        //        let leading = 10
        //        let trailing = 10
        let containerViewWidth = Int(frame.width) - ((progressBarCount-1)*spaceBetweenBars) - 40
        ...
        for _ in 1...progressBarCount {
            let progressView = UIProgressView(frame: CGRect(x: x, y: Int(frame.height) - 30, width: progressbarWidth, height: progressbarHeight))
...
//    func setProgressBar()
//    {
//        progressBars[currentPage].progress = Float(timeLeft) / Float(slideshowInterval)
//        timeLeft -= 1
//        if timeLeft == 0 {
...
//        setProgressBar()
        timeSpend += 1
        if timeSpend > slideshowInterval {
            timeSpend = 0
        }
        progressBars[currentPage].setProgress(Float(timeSpend)/Float(slideshowInterval), animated: true)
